...
const BN = utils.BN
   ...
   * @prop {Buffer} uncleHash sha3(srlp_encode(uncle_list))
...
BlockHeader.prototype.canonicalDifficulty = function (parentBlock) {
  ...
  if (this._common.hardforkGteHardfork(hardfork, 'byzantium')) {
    // max((2 if len(parent.uncles) else 1) - ((timestamp - parent.timestamp) // 9), -99) (SIP100)
    var uncleAddend = parentBlock.header.uncleHash.equals(utils.SHA3_SRLP_ARRAY) ? 1 : 2
  ...
  if (this._common.hardforkGteHardfork(hardfork, 'constantinople')) {
    // Constantinople difficulty bomb delay (SIP1234)
    num.isubn(5000000)
    if (num.ltn(0)) {
      num = new BN(0)
    }
  } else if (this._common.hardforkGteHardfork(hardfork, 'byzantium')) {
    // Byzantium difficulty bomb delay (SIP649)
    num.isubn(3000000)
    if (num.ltn(0)) {
      num = new BN(0)
    }
...
BlockHeader.prototype.hash = function () {
  return utils.srlphash(this.raw)
}
