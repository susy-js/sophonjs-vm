...
const Tx = require('sophonjs-tx')
const Trie = require('merkle-patricia-tree')
const BN = sofUtil.BN
const srlp = sofUtil.srlp
 ...
 * @param {Object} opts.common Alternatively pass a Common instance (sophonjs-common) instead of setting chain/hardfork directly
 ...
 * @method serialize
 * @param {Boolean} srlpEncode whether to srlp encode the block or not
 */
Block.prototype.serialize = function (srlpEncode) {
  var raw = [this.header.raw, [],
    []
  ]

  // srlpEnode defaults to true
  if (typeof srlpEncode === 'undefined') {
    srlpEncode = true
  }
  ...
  return srlpEncode ? srlp.encode(raw) : raw
}
...
Block.prototype.genTxTrie = function (cb) {
  ...
  async.eachSeries(this.transactions, function (tx, done) {
    self.txTrie.put(srlp.encode(i), tx.serialize(), done)
    i++
  }, cb)
}
...
Block.prototype.validateTransactionsTrie = function () {
  ...
  if (this.transactions.length) {
    ...
    return txT === sofUtil.SHA3_SRLP.toString('hex')
  }
}
...
Block.prototype.validateUnclesHash = function () {
  ...
  raw = srlp.encode(raw)
  return sofUtil.sha3(raw).toString('hex') === this.header.uncleHash.toString('hex')
}
...
Block.prototype.toJSON = function (labeled) {
  if (labeled) {
    ...
    return sofUtil.baToJSON(this.raw)
  }
}
