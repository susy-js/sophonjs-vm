...
module.exports = function (opts, cb) {
  ...
  function loadToAccount(done) {
    ...
    if (!toAddress) {
      ...
      if (salt) {
        createdAddress = toAddress = sofUtil.generateAddress2(caller, salt, code);
      } else {
        createdAddress = toAddress = sofUtil.generateAddress(caller, newNonce.toArray());
      }
  ...
  function checkAccountState(address, next, done) {
    stateManager.getAccount(address, function (err, account) {
      ...
      if (account.nonce && new BN(account.nonce) > 0 || account.codeHash.compare(sofUtil.KECCAK256_NULL) !== 0) {
        toAccount = account;
  ...
  function subTxValue(cb) {
    ...
    stateManager.putAccount(sofUtil.toBuffer(caller), account, cb);
  }
  ...
  function addTxValue(cb) {
    ...
    stateManager.putAccount(sofUtil.toBuffer(toAddress), toAccount, cb);
  }
      ...
      if (err) {
        ...
        stateManager.revert(function (revertErr) {
          if (revertErr || !isCompiled) cb(revertErr);else {
            ...
            // because the bug in both Graviton and Susy led to deleting RIPEMD precompiled in this case
            // see https://octonion.institute/susy-go/susy-graviton/pull/3341/files#diff-2433aa143ee4772026454b8abd76b9dd
