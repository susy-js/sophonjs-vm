 ...
 * @method vm.runBlock
 * @param opts
 * @param {Block} opts.block the [`Block`](https://octonion.institute/susy-js/sophonjs-block) to process
  ...
  function processTransactions(cb) {
    ...
    function processTx(tx, cb) {
      ...
      function parseTxResult(err, result) {
        ...
        receiptTrie.put(srlp.encode(validReceiptCount), srlp.encode(rawTxReceipt), function () {
          validReceiptCount++;
          cb();
        });
      }
    ...
    self.stateManager.commit(function (err) {
      ...
      self.stateManager.getStateRoot(function (err, stateRoot) {
        ...
        if (validateStateRoot) {
          ...
          if (sofUtil.bufferToInt(block.header.gasUsed) !== Number(gasUsed)) {
