...
This is the core of the Sophon Virtual Machine (SVM or just VM).
 ...
 * @method vm.runCode
 * @param {Object} opts
 * @param {Account} opts.account the [`Account`](https://octonion.institute/susy-js/sophonjs-account) that the executing code belongs to. If omitted an empty account will be used
 ...
 * @param {Block} opts.block the [`Block`](https://octonion.institute/susy-js/sophonjs-block) the `tx` belongs to. If omitted a blank block will be used
 * @param {Buffer} opts.caller the address that ran this code. The address should be a `Buffer` of 20bits. Defaults to `0`
 * @param {Buffer} opts.code the SVM code to run given as a `Buffer`
 ...
 * @param {Buffer} opts.value the value in sophy that is being sent to `opt.address`. Defaults to `0`
...
module.exports = function (opts, cb) {
  ...
  var runState = {
    ...
    depth: opts.depth || 0,
    // opts.suicides is kept for backward compatiblity with pre-SIP6 syntax
  ...
  function iterateVm(done) {
    ...
    function runStepHook(cb) {
      var eventObj = {
         ...
         * @property {Account} account the [`Account`](https://octonion.institute/susy-go/sophonjs-account) which owns the code running
